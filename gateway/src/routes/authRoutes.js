...
router.post("/employee-details", async (req, res) => {
  try {
    const backendRes = await fetch(`${AUTH_SERVICE}/api/auth/employee-details`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(req.body),
    });
...
router.get("/employee-details/:userId", async (req, res) => {
  try {
    const backendRes = await fetch(`${AUTH_SERVICE}/api/auth/employee-details/${req.params.userId}`, {
      method: "GET",
      headers: { "Content-Type": "application/json" },
    });
...
// Delete employee (deletes both User and EmployeeDetail)
